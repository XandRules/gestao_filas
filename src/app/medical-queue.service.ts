...
export class MedicalQueueService {
  ...
  private readonly resource = 'medicalQueue';
  ...
  async removeById(id: string): Promise<void> {
    try { await fetch(`${this.baseUrl}/${this.resource}/${id}`, { method: 'DELETE' });
  ...
  async clear(): Promise<void> {
    ...
    for (const id of ids) { try { await fetch(`${this.baseUrl}/${this.resource}/${id}`, { method: 'DELETE' }); } catch {} }
    this._queue = [];
  }
}